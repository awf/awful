classdef au_bsx
  ...
  %        The term "broadcast" comes from Python. The name "bsx" comes
  %        from matlab's "bsxfun", where "sx" means "singleton expansion"
  %        The examples below should, I hope, show that expressions with
  %        AU_BSX are much easier to read than the bsxfun equivalents.
  %      EXAMPLE
  %        a = randn(5,1); % Vector a
  %        B = randn(5,7); % Matrix B
  % 
  %      EXPRESSION:              = BSXFUN VERSION:
  %        au_bsx(a) + B          = bsxfun(@plus, a, B)
  %        max(au_bsx(a) + B, B)  = bsxfun(@max, bsxfun(@plus, a, B), B)
  ...
  methods
    function obj = au_bsx(c)
      % Construct a bsx object from a bsx object or a value 
      if isa(c,'au_bsx')
        obj.val= c.val;
      else
        obj.val = c;
