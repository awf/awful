...
for ki=1:p
  ...
  if opts.PatternOnly
    ...
    errscale = max([norm(fdJcol), norm(J(:,k)), opts.tol]);
    err = max(abs(fdJcol - J(:,k)))/errscale;
  end
  if err > opts.tol
    err = full(err);
    fprintf(2, '\nau_check_derivatives: Error on parameter %d = %g fd/colnorm = %g/%g (%dth checked)', ...
      k, err, norm(fdJcol), norm(J(:,k)), ki);
